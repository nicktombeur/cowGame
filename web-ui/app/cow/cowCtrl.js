    ...
    ctrls.controller('CowController', ['$scope', 'cowService', function ($scope, cowService) {
        ...
        vm.submit = function(valid){
            if(valid){
                ...
                var cube = scene.getObjectByName(vm.selectedCube.name);
        ...
        function mouseMove(event) {
            ...
            mouse2D.y = -( event.clientY / (window.innerHeight + ($("nav").height() * 2)) ) * 2 + 1;
        }

        function mouseClick(event) {
            event.preventDefault();
            if (!$('#modal3').is(":visible")) brushAction();
        ...
        function brushAction() {
            ...
            if (brush.mode == "select") {
                ...
                if (cube && !cube.selected) {
                    vm.selectedCube = {name: cube.name,x:cube.coord.x,y:cube.coord.y};
                    $('#modal3').openModal();
        ...
        function generateGrid() {
            ...
            for (var x = 0; x < GRID_SIZE; x++) {
                for (var z = 0; z < GRID_SIZE; z++) {
                    ...
                    if (cowService.getSelectedCubes(cube)) {
                        cube.material = materials["solid"][0];
                    } else {
                        cube.material = materials["solid"][1];
                    }
            ...
            if (!$('#modal3').is(":visible")) {
                ...
                if (result) {
                    if ((brush.mode == "select") && !result.object.base) {
                        ...
                        if(!targetCube.selected) targetCube.material = materials["select"][targetCube.colorIndex];
